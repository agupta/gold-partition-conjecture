...
def certificate_kind(up: tuple[int, ...]) -> str:
    """Return the order-independent `chain`, `pair`, `triple`, or `open`
    class used by the paper."""
    n = len(up)
    down = downsets_from_up(up)
    incomparable = [
        (x, y)
...
def has_certificate(up: tuple[int, ...]) -> bool:
    return certificate_kind(up) != "open"
...
def main() -> None:
    ...
    for n in range(1, maximum_order + 1):
        ...
        for up in posets[n]:
            ...
            open_classes += certificate_kind(up) == "open"
